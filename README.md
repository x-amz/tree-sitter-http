...
- **Body termination and typing** — a body runs to EOF as one opaque `body` node; no `###`, blank line, or `HTTP/` status line ends it, and its trailing blank lines are its octets. Neither dialect types a body; its language is the injection query's to find, by one rule in both (`queries/http/injections.scm`, `queries/http_message/injections.scm`): a message that declares a Content-Type has said what its body is — a media type the query knows (json, xml, html, form-urlencoded, `message/http`) is a directive, the text not consulted, and one it does not know names nothing — and a message that declares none is sniffed, its body's first line read for the same languages and no others. The message holds its Content-Type header in the field `content_type`, decided by the scanner's zero-width `_content_type_start`, so "declares none" is `!content_type` and no two patterns claim one body. In the file dialect a `body` is lines of text with placeholders read inside it, and `file_body` — `< path`, the format's own syntax, naming a file to send — is the one first line the grammar reads; its sniff differs from the wire's where the format does: a `{{` opens a placeholder, not JSON, and `<@` is not a tag. In the file dialect a body keeps its blank lines and ends at the last of them before `###`, EOF, or a status line at the margin — the response — on either side of an exchange; the scanner's `_body_blank` reads past a blank run to decide whether the body goes on.
...
Whitespace is structure: there are no `extras`, and every space, blank line and line end is a token, under one rule. `_ws` is the whitespace between two tokens on a line, hidden; `_eol` ends the line and owns the whitespace before it; a visible token never begins or ends with whitespace — `web/check.js` holds every document and corpus input to that. A header's `value` runs on across indented lines, and each line break in it, with the indentation after it, is a `fold` node: exactly the bytes that are not the value's, so a value's text is its range with each `fold` cut out and nothing put in its place. Every line-shaped rule ends in `$._eol`, an external token, a newline or zero-width at EOF. (Matching `"\0"` from the grammar looks like it works in one code path and not another; don't.) Most grammar bugs are a line lexing as the wrong kind. `PREC` at the top of `define-grammar.js` is the ladder that decides it; read its comments before touching a number. The guide's lex step lists, for the token under the caret, every token that was valid there and which won. The request, response and body rules are `prec.right` so a trailing comment, blank line or brace-led line attaches to them.
